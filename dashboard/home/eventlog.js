...
fetch(`/api/v1/events`)
  ...
  .then((response) => {
    ...
    eventlastResourceVersion = response.metadata.resourceVersion
    eventLogs.forEach((eventLog) => {
      const eventLogID = `${eventLog.metadata.namespace}-${eventLog.metadata.name}`
      eventapp.upsert(eventLogID, eventLog)
...
function streamUpdates(apiString) {
  fetch(`/api/v1/events?watch=1&resourceVersion=${eventlastResourceVersion}`)
    .then((response) => {
        ...
        const remainingBuffer = findLine(buffer, (line) => {
          try {
            ...
            switch (event.type) {
              ...
              case 'DELETED': {
                eventapp.remove(eventLogID, eventLog)
                break
              }
              case 'MODIFIED': {
                eventapp.upsert(eventLogID, eventLog)
            ...
            eventlastResourceVersion = event.object.metadata.resourceVersion
          } catch (error) {
